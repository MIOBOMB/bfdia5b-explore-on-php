const serverReq = location.origin + location.pathname + 'server/'; // Server address
const serverSource = 'https://coppersalts.github.io/HTML5b/'; // url to all game content
const menuMusic = 'https://coppersalts.github.io/HTML5b/data/the fiber 16x loop.wav'; // link to menu music, you can disable by set empty string

const loginPage = location.origin + location.pathname + 'login.php'; // page where you register your account
let serverLogin = 'serverLogin'; // once you need to rename this variable in "server/api.php", else login and register won't work
let serverUser = localStorage.getItem(serverLogin); // user token
...
if (typeof navigator.clipboard !== 'undefined') {
	var browserPasteSolution = typeof navigator.clipboard.readText === "function";
	var browserCopySolution = typeof navigator.clipboard.write === "function";
} else {
	var browserPasteSolution = false;
	var browserCopySolution = false;
}
...
let [levelId, levelpackId] = [searchParams.get("https://cfmot.ru/?level"), searchParams.get("https://cfmot.ru/?levelpack")]
const difficultyMap = [
	["Unknown", "#e6e6e6"],
	["Easy", "#85ff85"],
	["Normal", "#ffff00"],
...
let loggedInExploreUser = -1; // Temporarily just being used for checking if the user is logged in.
...
let musicSound = !!menuMusic ? new Audio(menuMusic):0;
//let musicSound = new Audio('data/map01.mp3');
...
async function loadingScreen() {
	...
	let req = await fetch(serverSource+'data/levels.txt');
	levelsString = await req.text();
	loadLevels();

	req = await fetch(serverSource+'data/images6.json');
...
function exploreCopyLink() {
	copyText('https://cfmot.ru/HTML5b/?' + (exploreLevelPageType===0?'level=':'levelpack=') + exploreLevelPageLevel.id);
}
...
function drawAnyText(text) {
	ctx.font = 'bold 45px Helvetica';
	ctx.textAlign = 'center';
	ctx.textBaseline = 'middle';
	ctx.fillStyle = '#ffffff';
	ctx.fillText(text, 560, 60);
}
...
function setup() {
	...
	if (serverUser) loggedInExploreUser = serverUser;

	if (levelId) {
		// If the level ID is specified in the URL, load that level.
		menuScreen = 0;
		exploreLevelPageType = 0;
		fetch(serverReq+'level/?id=' + levelId, {method: 'GET'})
			.then(async (res) => {
		...
		menuScreen = 1;
		exploreLevelPageType = 1;
		fetch(serverReq+'levelpack/?levels=1&id=' + levelpackId, {method: 'GET'})
			.then(async (res) => {
				exploreLevelPageLevel = await res.json();
	...
	switch (menuScreen) {
		...
		case 7:
			...
			if (exploreLoading) {
				...
				if (exploreLevelPageType != 1 && loggedInExploreUser === exploreLevelPageLevel.creator.id) {
...
function getExplorePage(p, t, s) {
	requestAdded();
	return newHelperJS_request(serverReq+'page/?page=' + p + '&sort=' + s + '&type=' + t)
		.then(response => {
			explorePageLevels = JSON.parse(response);
...
function getSearchPage(searchText, p) {
	requestAdded();
	return newHelperJS_request(serverReq+'search/?text=' + encodeURIComponent(searchText).replace('%20','+') + '&page=' + p)
		.then(response => {
			explorePageLevels = JSON.parse(response);
			setExploreThumbs();
			truncateLevelTitles(explorePageLevels,0);
...
function getExploreLevel(id) {
	requestAdded();
	return newHelperJS_request(serverReq+'level/?id=' + id)
		.then(response => {
			exploreLevelPageLevel = JSON.parse(response);
			drawExploreThumb(thumbBigctx, thumbBig.width, exploreLevelPageLevel.data, 0.4);
			requestResolved();
		})
		.catch(err => {
...
function getExploreLevelpack(id) {
	requestAdded();
	return newHelperJS_request(serverReq+'levelpack/?levels=1&id=' + id)
		.then(response => {
			exploreLevelPageLevel = JSON.parse(response);
...
function getExplorePlay(id) {
	...
	return newHelperJS_request(`${serverReq}play/?type=${exploreLevelPageType}&id=${id}`)
}

function getExploreUser(id) {
	requestAdded();
	return newHelperJS_request(serverReq+'user/?id=' + id)
		.then(async response => {
...
function getCurrentExploreUserID() {
	return loggedInExploreUser = serverUser
	return fetch('https://discord.com/api/v10/users/@me', {
		method: 'GET',
		headers: {'Authorization': 'Bearer ' + getCookie('access_token')}
	}).then(async indentity => {
		const discordId = (await indentity.json()).id;
		fetch(serverReq+'user/?discordId=' + discordId, {method: 'GET'})
			.then(async response => {
				loggedInExploreUser = (await response.json()).id;
			document.cookie = '5beam_id=' + loggedInExploreUser + '; path=/';
...
function getExploreUserPage(id, p, t, s) {
	requestAdded();
	return newHelperJS_request(serverReq+'user/page/?id=' + id + '&page=' + p + '&type=' + t + '&sort=' + s)
		.then(response => {
			exploreUserPageLevels[t] = JSON.parse(response);
...
function refreshToken() {
	newHelperJS_request(serverReq+'auth/refresh/', 'refresh_token='+serverUser)
	.then(data => {
		switch (data) {
			case '200':
				loggedInExploreUser = serverUser;
...
function logInExplore() {
	...
	document.body.insertAdjacentHTML(
		'beforeend',
		`<div id=loginPage>`+
			`<button onclick=dropLogin() style=position:absolute;top:5px;right:5px;>X</button>`+
			`<iframe src="${loginPage}" frameborder=0 height=100% width=100%></iframe>`+
		`</div>`
	);

	window.addEventListener('message', function(event) {
		serverUser = event.data;
		refreshToken();
		dropLogin();
	});
}
...
function logOutExplore() {
	loggedInExploreUser = -1;
	bfdia5b.removeItem(serverLogin);
	serverUser = null;
	return
...
function postExploreLevelOrPack(title, desc, data, isLevelpack=false) {
	console.log(isLevelpack)
	...
	const postData = 
	`access_token=${serverUser}&title=${title}&description=${desc}&file=${data}`
	console.log(postData);

	newHelperJS_request(serverReq+'create/' + (isLevelpack?'levelpack/':'level/'), postData)
		.then(response => {
			...
			if (response == '1') {
				...
				setLCMessage('Server responded ' + response);
			}
		})
		.catch(err => {
			console.log(err);
...
function postExploreModifyLevel(id, title, desc, difficulty, file) {
	...
	return fetch(serverReq+'modify/level/?id=' + id, {method: 'POST', body: JSON.stringify(body)})
		.then(response => {
			requestResolved();
...
if (bfdia5b.getItem(serverLogin) != null) {
	refreshToken();
}

function newHelperJS_request(url, data = 0) {
